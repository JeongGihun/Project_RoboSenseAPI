...
## 시나리오
```
물류 창고에서 자율주행 로봇 여러 대가 물건을 운반하면서 실시간으로 센서 데이터를 API 서버로 전송.
서버는 이를 처리하여 로봇의 위치 추적, 장애물 감지, 경로 최적화 등을 지원.
```
...
## 목표
- 대규모 트래픽 처리 경험 쌓기 위해 극한 조건 설정 및 성능 최적화
- 평가 지표 : TPS, 응답 지연 시간, 에러율, 최대 동시 접속자

## 실험 및 관찰 방법
- 실험 : Locust로 점진적 부하(100 → 500 → 1,000 → 3,000 → 5,000...)
- 관찰 : 평가 지표에 맞춰 그래프로 표현
- 문제 발생 → 가설 → 적용 → 측정 → 다음 문제 반복
